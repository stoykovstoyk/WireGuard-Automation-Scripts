...
# === Prompt for input ===
read -rp "Enter client name (e.g., alice): " CLIENT_NAME
...
# Get next available IP and set as default
DEFAULT_IP=$(find_next_available_ip)

# Prompt for client IP with default value
read -rp "Enter client IP (e.g., 10.0.0.2) [default: $DEFAULT_IP]: " CLIENT_IP
...
if ! is_ip_available "$CLIENT_IP"; then
    echo "[ERROR] IP address $CLIENT_IP is already in use"
...
if grep -q "$CLIENT_PUBLIC_KEY" "$WG_CONFIG"; then
    echo "[!] Client already exists in WireGuard config. Aborting to avoid duplicate."
    exit 1
fi
...
# === Optional QR Code ===
read -rp "Generate QR code for mobile client? (y/n) [n]: " QRCODE_CHOICE

# Use default if user pressed enter
if [[ -z "$QRCODE_CHOICE" ]]; then
    QRCODE_CHOICE="n"
fi

if [[ "$QRCODE_CHOICE" =~ ^[Yy]$ ]]; then
    if ! command -v qrencode &>/dev/null; then
        echo "[!] qrencode not found. Installing..."
        apt install -y qrencode
    fi
    qrencode -o "$CLIENTS_DIR/${CLIENT_NAME}.png" < "$CLIENT_CONF"
    echo "[+] QR code saved to: $CLIENTS_DIR/${CLIENT_NAME}.png"
fi
...
[[ "$QRCODE_CHOICE" =~ ^[Yy]$ ]] && echo "➡ QR code:    $CLIENTS_DIR/${CLIENT_NAME}.png"
echo ""
